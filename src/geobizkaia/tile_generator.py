...
class TileGenerator:
    ...
    def __init__(self, tile_size=512, overlap=0.2):
        ...
        logger.info(
            f"Tile: size={tile_size}, overlap={overlap*100:.0f}%, stride={self.stride}")
        ...
        with rasterio.open(image_path) as src:
            ...
            for tile_idx, (y_start, x_start, y_end, x_end) in enumerate(coords):
                ...
                if tile_height < self.tile_size or tile_width < self.tile_size:
                    padded = np.zeros(
                        (image.shape[0], self.tile_size, self.tile_size), dtype=image.dtype)
    ...
    def tile_mask(self, mask_path, output_dir, prefix):
        ...
        for tile_idx, (y_start, x_start, y_end, x_end) in enumerate(coords):
            ...
            if tile_height < self.tile_size or tile_width < self.tile_size:
                padded = np.zeros(
                    (self.tile_size, self.tile_size), dtype=mask_array.dtype)
                padded[:tile_height, :tile_width] = tile_data
                tile_data = padded
    ...
    def tile_dataset(self, images_dir, masks_dir, output_images_dir, output_masks_dir, prefix="tile"):
        ...
        image_files = sorted(
            f for f in list(images_dir.glob("*.tif")) + list(images_dir.glob("*.tiff"))
            if "_tile_" not in f.stem
        )
        ...
        for image_path in image_files:
            ...
            tile_count += 1
            total_tiles += len(self.generate_tile_coords(*
        ...
        logger.info(
            f"Tiled {tile_count} pairs into {total_tiles} tiles (originals removed)")
